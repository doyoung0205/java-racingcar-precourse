...
## 기능구현 목록
- [ ] `레이싱카`는 5자 이하의 이름, 전진한 횟수를 가진다.
  - [ ] 이름이 5자를 초과할 경우, `[ERROR] 경주할 자동차의 이름이 5자 초과하였습니다.` 라는 에러가 발생 한다.
- [ ] `레이싱카`는 라운드 마다 `레이스`를 할 수 있다.
  - [ ] `레이스`는 0에서 9 사이에서 random 값을 구한 후 random 값이 4 이상일 경우 전진하고, 3이하의 값이면 멈춘다.
- [ ] `레이싱카그룹`은 쉼표(`,`)를 기준으로 구분하여 `레이싱카` 를 생성 할 수 있다.
- [ ] `레이싱카게임`은 경주에 참여할 `레이싱카그룹` 을 생성할 수 있다.
- [ ] `레이싱카게임`은 모든 라운드가 끝나면 종료된다.
- [ ] `레이싱카게임`은 종료시 후 우승한 `레이싱카그룹`을 알 수 있다.

---

- [ ] `레이싱카게임 컨트롤러`는 레이싱카 이름들과 라운드 횟수를 통해  `레이싱카게임` 을 생성 할 수 있다.
  - [ ] 콘솔로 부터 `경주할 자동차 이름을 입력하세요. (이름은 쉼표(,) 기준으로구분)` 라는 메시지와 함께 `경주할 자동차 이름들`을 받을 수 있다.
...
| 레이싱카그룹 | racingCarGroup | 레이싱카게임의 참여하는 레이싱카 리스트 |
| 레이스 | racing | 라운드의 레이싱을 하는 행위 | 
| 우승 | victory | 레이싱카게임 에서 가장 전진횟수가 많은 레이싱카 |
