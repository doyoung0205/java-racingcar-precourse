...
class RoundScoreGroupsTest {
    ...
    void 마지막_라운드의_최대_점수를_받은_레이싱카그룹과_전체_라운드의_최대_점수를_받은_레이싱카그룹과_같다(int length) {
        // given
        ...
        for (int i = 0; i < length; i++) {
            ...
            lastRoundRacingCarCaptureGroups = updateRacingCarCaptureGroupsIfLastRound(lastRoundRacingCarCaptureGroups, length, i, roundScore);
        }
        final RoundScoreGroups roundScoreGroups = RoundScoreGroups.valueOf(beforeRoundScores);
        // when
    ...
    private void assertEqualsRacingCarCaptureGroups(RacingCarCaptureGroups lastRoundRacingCarCaptureGroups, RacingCarCaptureGroups victoryGroups) {
        final List<RacingCarCapture> racingCarCaptures = victoryGroups.getRacingCarCaptures();
        final List<RacingCarCapture> lastRoundRacingCarCaptures = lastRoundRacingCarCaptureGroups.getRacingCarCaptures();
        for (int i = 0; i < lastRoundRacingCarCaptures.size(); i++) {
            assertEquals(lastRoundRacingCarCaptures.get(i), racingCarCaptures.get(i));
        }
    }

    private RacingCarCaptureGroups updateRacingCarCaptureGroupsIfLastRound(RacingCarCaptureGroups lastRoundRacingCarCaptureGroups, int length, int i, RoundScore roundScore) {
        if (i == length - 1) {
